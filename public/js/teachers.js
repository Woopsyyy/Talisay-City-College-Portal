...
const heroSpotlights = {
    ...
    evaluation: {
        hero_copy: 'View your evaluation statistics and feedback from students to improve your teaching.',
        spotlight_eyebrow: 'Performance insights',
        spotlight_title: 'Evaluation Statistics',
        spotlight_copy: 'Review detailed evaluation statistics, student feedback, and recommendations to enhance your teaching effectiveness.'
    },
    settings: {
    ...
    grade_system: {
        hero_copy: 'Input and manage student grades for your assigned subjects efficiently.',
        spotlight_eyebrow: 'Grade management',
        spotlight_title: 'Grade System',
        spotlight_copy: 'Manage student grades, track academic progress, and maintain accurate records for all your classes.'
    }
};
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (imgEl) {
            ...
            } else if (imagePath && imagePath.startsWith('/TCC/database/pictures/')) {
        ...
        if (roleEl) {
            roleEl.textContent = session.user.role ? session.user.role.charAt(0).toUpperCase() + session.user.role.slice(1) : 'Teacher';
    ...
    try {
        switch (view) {
            ...
            case 'grade_system':
                ...
                break;
            default:
                contentArea.innerHTML = '<div class="alert alert-danger">View not found</div>';
        }
    } catch (error) {
...
function updateHeroContent(view) {
    ...
    if (spotlightEyebrowEl) spotlightEyebrowEl.textContent = spotlight.spotlight_eyebrow;
    if (spotlightTitleEl) spotlightTitleEl.textContent = spotlight.spotlight_title;
        ...
        if (Object.keys(schedulesByDay).length === 0) {
            ...
            sortedDays.forEach(day => {
                ...
                daySchedules.forEach(schedule => {
                    const timeStart = schedule.time_start ? new Date('1970-01-01T' + schedule.time_start).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true }) : 'N/A';
                    const timeEnd = schedule.time_end ? new Date('1970-01-01T' + schedule.time_end).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true }) : 'N/A';
                    const timeRange = timeStart !== 'N/A' && timeEnd !== 'N/A' ? `${timeStart} - ${timeEnd}` : 'N/A';
...
async function loadAnnouncementsView() {
    ...
    try {
        ...
        try {
            ...
            contentArea.innerHTML = `<div class="alert alert-danger">Error loading announcements: ${escapeHtml(apiError.message || 'Unknown error')}</div>`;
            return;
        }
        
        // Filter announcements
        let filteredAnnouncements = Array.isArray(announcements) ? announcements : [];
        if (filterYear) {
            filteredAnnouncements = filteredAnnouncements.filter(a => a.year == filterYear);
        }
        if (filterDept) {
        ...
        if (filterMajor) {
            filteredAnnouncements = filteredAnnouncements.filter(a => a.major === filterMajor);
        }
        ...
        const majorsForDept = (filterDept && departmentMajors[filterDept]) ? departmentMajors[filterDept] : [];
                        ...
                        <form method="get" class="announcements-filter-form" id="announcementsFilterForm">
                            ...
                            <div class="filter-group">
                                ...
                                <select id="dept_filter" name="dept_filter" class="filter-select">
                                    ...
                                    <option value="HM" ${filterDept === 'HM' ? 'selected' : ''}>HM</option>
                                    <option value="BSED" ${filterDept === 'BSED' ? 'selected' : ''}>BSED</option>
                                    <option value="BEED" ${filterDept === 'BEED' ? 'selected' : ''}>BEED</option>
                                    <option value="TOURISM" ${filterDept === 'TOURISM' ? 'selected' : ''}>TOURISM</option>
                                </select>
                            </div>
                            <div class="filter-group">
                                ...
                                <select id="major_filter" name="major_filter" class="filter-select" ${majorsForDept.length === 0 ? 'disabled' : ''}>
                                    <option value="">All Majors</option>
                                    ${majorsForDept.map(major => `<option value="${escapeHtml(major)}" ${filterMajor === major ? 'selected' : ''}>${escapeHtml(major)}</option>`).join('')}
                                </select>
                            </div>
        ...
        if (filteredAnnouncements.length === 0) {
            ...
            filteredAnnouncements.forEach(a => {
                ...
                const formattedDate = a.date ? new Date(a.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : 'Date not specified';
                
                html += `
                    <div class="announcement-card-modern">
                        <div class="announcement-card-header">
                            <div class="announcement-title-section">
                                <h4 class="announcement-title">${escapeHtml(a.title || 'Untitled')}</h4>
                        ...
                        <div class="announcement-footer">
                            ${a.year ? `<span class="announcement-badge"><i class="bi bi-mortarboard"></i> ${yearNum > 0 ? formatOrdinal(yearNum) + ' Year' : escapeHtml(a.year)}</span>` : ''}
                            ${deptLabel ? `<span class="announcement-badge"><i class="bi bi-building"></i> ${escapeHtml(deptLabel)}</span>` : ''}
        ...
        if (deptFilter) {
            deptFilter.addEventListener('change', (e) => {
                ...
                const majors = (dept && departmentMajors[dept]) ? departmentMajors[dept] : [];
                
                if (majorSelect) {
                    majorSelect.innerHTML = '<option value="">All Majors</option>';
                    majors.forEach(major => {
                        majorSelect.innerHTML += `<option value="${escapeHtml(major)}">${escapeHtml(major)}</option>`;
                    });
        ...
        contentArea.innerHTML = '<div class="alert alert-danger">Error loading announcements: ' + escapeHtml(error.message) + '</div>';
    }
}
    ...
    try {
        ...
        if (projectsList.length === 0) {
            ...
            projectsList.forEach(proj => {
                const isCompleted = proj.completed && String(proj.completed).toLowerCase() === 'yes';
                html += `
                    <div class="project-card-modern">
                        <div class="project-card-header">
                            <div class="project-title-section">
                                <h4 class="project-title">${escapeHtml(proj.name || 'Untitled')}</h4>
                        ...
                        <div class="project-details">
                            ...
                            <div class="project-detail-item">
                                ...
                                <div class="project-detail-value">${formatFriendlyDate(proj.started)}</div>
                            </div>
                        </div>
                    </div>
                `;
...
async function loadEvaluationView() {
    ...
    try {
        ...
        const [stats, schedules] = await Promise.all([
            TeacherAPI.getEvaluationStatistics().catch(() => ({ success: false, total: 0 })),
        ...
        if (!stats.success || stats.total === 0) {
                ...
                <div class="info-card">
                    ...
                    <div class="evaluation-overview">
                        <div class="overview-stat highlight">
                            ...
                            <div class="stat-value">${(stats.overall?.average || 0).toFixed(2)}</div>
                            <div class="stat-subtext">${(stats.overall?.percentage || 0).toFixed(1)}% overall</div>
                        </div>
            ...
            part1Questions.forEach((question, index) => {
                ...
                const stat = stats.statistics?.[qKey] || { average: 0, percentage: 0, responses: 0 };
                html += `
                    <div class="stat-item">
                        ...
                        <div class="stat-metrics">
                            ...
                            <div class="metric">
                                ...
                                <span class="metric-value">${stat.responses || 0}</span>
                            </div>
                        </div>
                        <div class="stat-bar">
            ...
            const satisfaction = stats.statistics?.satisfaction || { average: 0, percentage: 0, responses: 0 };
            const recommendation = stats.statistics?.recommendation || { yes: 0, no: 0, percentage: 0, total: 0 };
                        ...
                        <div class="stat-item">
                            ...
                            <div class="stat-metrics">
                                <div class="metric">
                                    ...
                                    <span class="metric-value">${recommendation.yes || 0}</span>
                                </div>
                                <div class="metric">
                                    <span class="metric-label">No:</span>
                                    <span class="metric-value">${recommendation.no || 0}</span>
                                </div>
                                <div class="metric">
                            ...
                            <table class="table table-striped align-middle">
                                ...
                                <tbody>
                                    ${(Array.isArray(schedules) && schedules.length > 0 ? schedules : []).map(s => {
                                        const timeStart = s.time_start ? new Date('1970-01-01T' + s.time_start).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true }) : 'N/A';
                                        const timeEnd = s.time_end ? new Date('1970-01-01T' + s.time_end).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true }) : 'N/A';
                                        const timeRange = (timeStart !== 'N/A' && timeEnd !== 'N/A') ? `${timeStart} - ${timeEnd}` : 'N/A';
                                        return `
                                            <tr>
                                        ...
                                        `;
                                    }).join('') || '<tr><td colspan="5" class="text-muted">No schedule found.</td></tr>'}
                                </tbody>
                            </table>
                        </div>
                    </div>
...
async function loadSettingsView() {
    ...
    try {
        ...
        let imagePath = user.image_path || 'images/sample.jpg';
        if (imagePath && imagePath.startsWith('/TCC/public/')) {
            imagePath = imagePath.replace('/TCC/public/', '');
        } else if (imagePath && imagePath.startsWith('/TCC/database/pictures/')) {
            imagePath = imagePath.replace('/TCC/database/pictures/', 'database/pictures/');
                            ...
                            <div class="settings-profile-section">
                                <div class="profile-image-container">
                                    <div class="profile-image-wrapper">
                                        <img id="preview" src="${escapeHtml(imagePath)}" class="profile-preview-large" alt="Profile" />
                                ...
                                <div class="settings-form-fields">
                                    <div class="settings-field">
                                        ...
                                        <input id="username" name="username" class="settings-input" value="${escapeHtml(user.username || '')}" required />
                                    </div>
                                    <div class="settings-field">
                                        <label for="full_name" class="settings-label">
                                            <i class="bi bi-person-badge"></i> Full Name
                                        </label>
                                        <input id="full_name" name="full_name" class="settings-input" value="${escapeHtml(user.full_name || '')}" required />
                                    </div>
                                    <div class="settings-field">
                                        <label for="password" class="settings-label">
                                            <i class="bi bi-lock"></i> New Password (leave blank to keep current)
                                        </label>
                                        <input type="password" id="password" name="password" class="settings-input" />
                                    </div>
                                    <div class="settings-field">
...
async function loadGradeSystemView() {
    ...
    try {
        ...
        const availableSections = Array.isArray(sectionsResp?.sections) ? sectionsResp.sections : [];
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <select id="subjectFilter" class="form-select form-select-lg">
                                        ...
                                        ${subjects.map(s => `<option value="${escapeHtml(s.subject_code)}">${escapeHtml(s.subject_code)} - ${escapeHtml(s.title)}</option>`).join('')}
                                    </select>
                                </div>
                                <div class="col-md-6">
        ...
        function matchesSubject(gSubject, subjCode, subjTitle) {
            ...
            if (code && (gs === code || gs.startsWith(code) || gs.includes(code))) return true;
        ...
        function populateSections() {
            ...
            sectionSelect.innerHTML = '<option value="">All Sections</option>' + sectionsForSubj.map(sec => `<option value="${escapeHtml(sec)}">${escapeHtml(sec)}</option>`).join('');
        ...
        async function applyFilters() {
            ...
            const filteredGrades = (grades || []).filter(g => {
                if (selectedSubject && !matchesSubject(g.subject, selectedSubject, subjTitle)) return false;
            ...
            let students = [];
            if (selectedSection) {
                gradesContainer.innerHTML = '<p class="text-muted">Loading students...</p>';
            ...
            students.forEach(stu => {
                const match = gradesCopy.find(g => (g.user_id && stu.id && g.user_id === stu.id) || (g.username && stu.username && g.username === stu.username));
                cards.push({
                    student: stu,
                    grade: match || null
                });
            ...
            gradesContainer.innerHTML = renderStudentGradeCardsMerged(cards, selectedSubject, selectedSection);
        }
...
function renderStudentGradeCardsMerged(items, selectedSubject, selectedSection) {
    ...
    const orderedYears = Object.keys(byYear).sort((a, b) => parseInt(a) - parseInt(b));
    ...
    orderedYears.forEach(yearNum => {
        ...
        byYear[yearNum].forEach(({ grade, student }) => {
            const key = grade.user_id || student.id || (grade.username || student.username || '').toLowerCase();
            if (!key) return;
            if (!groups[key]) {
                const displayName = grade.student_name || student.full_name || student.username || student.school_id || 'Student';
                const imagePath = (grade.image_path || student.image_path || '').replace('/TCC/public/','').replace('/TCC/','') || 'images/sample.jpg';
                groups[key] = {
                    display: displayName,
        ...
        Object.values(groups).forEach(group => {
            ...
            const summaryText = subjectCount > 0
                ? `${subjectCount} ${subjectCount === 1 ? 'subject' : 'subjects'} recorded`
                : 'No grades yet';
            const averageScore = scoredSubjects > 0
                ? Math.round((totalScores / scoredSubjects) * 10) / 10
                : null;

            const sem1 = semesterSummaries.find(s => s.label.toLowerCase().includes('1st')) || { average: null };
            const sem2 = semesterSummaries.find(s => s.label.toLowerCase().includes('2nd')) || { average: null };
                ...
                <div class="student-grade-card d-flex align-items-center justify-content-between" style="background:#fff; border-radius:14px; box-shadow: 0 6px 16px rgba(0,0,0,0.08); padding:14px 16px; border:1px solid #f1f1f1;">
                    <div class="d-flex align-items-center gap-3">
                        <div class="avatar flex-shrink-0" style="width:48px;height:48px;border-radius:12px;overflow:hidden;">
                            <img src="${escapeHtml(imagePath)}" alt="${escapeHtml(displayName)}" style="width:100%;height:100%;object-fit:cover;" onerror="this.src='images/sample.jpg'; this.onerror=null;">
                        </div>
                    ...
                    <div class="d-flex align-items-center gap-2 flex-wrap justify-content-end">
                        <span class="pill-chip pill-chip--sem1" title="1st Semester average">
                            1st Sem ${sem1.average !== null ? `<small>${sem1.average} avg</small>` : '<small>—</small>'}
                        </span>
                        <span class="pill-chip pill-chip--sem2" title="2nd Semester average">
                            2nd Sem ${sem2.average !== null ? `<small>${sem2.average} avg</small>` : '<small>—</small>'}
                        </span>
                        ${averageScore !== null ? `<span class="pill-chip pill-chip--accent">${averageScore}<small>avg</small></span>` : ''}
                        <button class="button2 btn-edit2" onclick="editGradeTeacher(${anyGradeId || '""'})"><i class="bi bi-pencil-square me-1"></i>Edit</button>
                        <button class="button2 btn-add2" onclick="openAddGradeModal({ username: '${escapeHtml(displayName)}', subject: '', section: '', year: '${escapeHtml(yearNum)}', semester: 'First Semester' })"><i class="bi bi-plus-lg me-1"></i>Add</button>
                        <button class="button2 btn-del2" onclick="deleteAllStudentGrades([${allGradeIds.join(',')}], '${escapeHtml(displayName)}')"><i class="bi bi-trash me-1"></i>Delete</button>
                    </div>
                </div>
            `;
        });
                    ...
                    <div class="modal-body" style="background:#fdfdfd;">
                        <form id="editGradeForm">
                            <div class="alert alert-info d-flex align-items-center gap-2 py-2">
                                ...
                                <div>
                                    <div class="fw-semibold mb-0">${escapeHtml(grade.student_name || grade.username || 'Student')}</div>
                                    <div class="text-muted small mb-0">${escapeHtml(grade.subject || 'Subject')} • ${escapeHtml(grade.section || 'Section')}</div>
                                </div>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" value="${escapeHtml(grade.student_name || grade.username || '')}" disabled>
                            </div>
                            <div class="mb-3">
                                <label class="form-label"><i class="bi bi-book me-1 text-primary"></i>Subject</label>
                                <input type="text" class="form-control" value="${escapeHtml(grade.subject || '')}" disabled>
                            </div>
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <label class="form-label"><i class="bi bi-calendar-week me-1 text-primary"></i>Year</label>
                                    <div class="form-control-plaintext fw-semibold">${escapeHtml(grade.year || 'N/A')}</div>
                                    <input type="hidden" id="editGradeYear" value="${escapeHtml(grade.year || '')}">
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label"><i class="bi bi-calendar-range me-1 text-primary"></i>Semester</label>
                                    <div class="form-control-plaintext fw-semibold">${escapeHtml(grade.semester || 'First Semester')}</div>
                                    <input type="hidden" id="editGradeSemester" value="${escapeHtml(grade.semester || 'First Semester')}">
                                </div>
                            </div>
                            <div class="row g-3 mt-1">
                                <div class="col-md-4">
                                    <label class="form-label"><i class="bi bi-123 me-1 text-primary"></i>Prelim Grade</label>
                                    <input type="number" step="0.01" id="editGradePrelim" class="form-control" value="${grade.prelim_grade || ''}">
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label"><i class="bi bi-123 me-1 text-primary"></i>Midterm Grade</label>
                                    <input type="number" step="0.01" id="editGradeMidterm" class="form-control" value="${grade.midterm_grade || ''}">
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label"><i class="bi bi-123 me-1 text-primary"></i>Finals Grade</label>
                                    <input type="number" step="0.01" id="editGradeFinals" class="form-control" value="${grade.finals_grade || ''}">
                                </div>
                            </div>
        ...
        document.getElementById('editGradeForm').addEventListener('submit', async (e) => {
            ...
            try {
                await TeacherAPI.updateGrade(grade.id, {
                    ...
                    prelim_grade: parseFloat(document.getElementById('editGradePrelim').value) || null,
                    midterm_grade: parseFloat(document.getElementById('editGradeMidterm').value) || null,
                    finals_grade: parseFloat(document.getElementById('editGradeFinals').value) || null
                });
                bsModal.hide();
                modal.remove();
                await loadGradeSystemView();
            ...
            <div class="modal-content">
                ...
                <div class="modal-body" style="background:#fdfdfd;">
                    <form id="addGradeForm">
                        <input type="hidden" id="addGradeUsername" value="${escapeHtml(prefill.username || '')}">
                        <input type="hidden" id="addGradeYear" value="${escapeHtml(prefill.year || '')}">
                        <input type="hidden" id="addGradeSubject" value="${escapeHtml(prefill.subject || '')}">
                        <input type="hidden" id="addGradeSection" value="${escapeHtml(prefill.section || '')}">
                        <input type="hidden" id="addGradeInstructor" value="${escapeHtml(prefill.instructor || '')}">

                        <div class="mb-3">
                            <div class="alert alert-info d-flex align-items-center gap-2 py-2">
                                <i class="bi bi-info-circle text-primary fs-5"></i>
                                <div class="d-flex flex-column gap-1 small mb-0">
                                    <div><strong>Student:</strong> ${escapeHtml(prefill.username || 'N/A')}</div>
                                    <div><strong>Subject:</strong> ${escapeHtml(prefill.subject || 'N/A')}</div>
                                    <div><strong>Section:</strong> ${escapeHtml(prefill.section || 'N/A')}</div>
                                    <div><strong>Semester:</strong> ${escapeHtml(prefill.semester || 'First Semester')}</div>
                                </div>
                            </div>
                            <input type="hidden" id="addGradeSemester" value="${escapeHtml(prefill.semester || 'First Semester')}">
                        </div>
                        <div class="row g-3">
                            <div class="col-md-4">
    ...
    document.getElementById('addGradeForm').addEventListener('submit', async (e) => {
        ...
        try {
            await TeacherAPI.createGrade({
                ...
                instructor: document.getElementById('addGradeInstructor').value || null,
                prelim_grade: parseFloat(document.getElementById('addGradePrelim').value) || null,
                midterm_grade: parseFloat(document.getElementById('addGradeMidterm').value) || null,
                finals_grade: parseFloat(document.getElementById('addGradeFinals').value) || null
            });
            bsModal.hide();
            modal.remove();
            await loadGradeSystemView();
...
async function deleteAllStudentGrades(ids = [], name = 'this student') {
    ...
    if (!confirm(`Delete ${ids.length} grade(s) for ${name}? This cannot be undone.`)) return;
    try {
        for (const id of ids) {
            if (id) {
                await TeacherAPI.deleteGrade(id);
