...
const heroSpotlights = {
    ...
    evaluation: {
        hero_copy: 'Evaluate your teachers and provide feedback to help improve the learning experience for everyone.',
        spotlight_eyebrow: 'Teacher evaluation',
        spotlight_title: 'Teacher Evaluation',
        spotlight_copy: 'Share your thoughts about your teachers and help improve the quality of education through constructive feedback.'
    },
    settings: {
...
function updateSidebar() {
    if (currentUser) {
        ...
        if (sidebarImage) {
            ...
            console.log('updateSidebar - final imagePath:', imagePath);
            ...
            sidebarImage.onerror = function() {
                console.error('Failed to load image:', imagePath, 'falling back to sample.jpg');
        ...
        if (sidebarSchoolId) {
            ...
            const isValidSchoolId = currentUser.school_id && 
...
function setupNavigation() {
    ...
    window.addEventListener('popstate', (e) => {
        const view = e.state?.view || new URLSearchParams(window.location.search).get('view') || 'records';
        loadView(view);
    });
}
...
function updateHeroContent(view) {
    ...
    document.getElementById('heroUserName').textContent = currentUser?.full_name || currentUser?.username || 'User';
    document.getElementById('heroCopy').textContent = spotlight.hero_copy;
    document.getElementById('spotlightEyebrow').textContent = spotlight.spotlight_eyebrow;
    document.getElementById('spotlightTitle').textContent = spotlight.spotlight_title;
    document.getElementById('spotlightCopy').textContent = spotlight.spotlight_copy;
}
    ...
    try {
        ...
        if (assignment) {
            ...
            if (assignment.year && assignment.section) {
                const sectionAssignment = (Array.isArray(sectionAssignments) ? sectionAssignments : []).find(sa => 
                ...
                if (sectionAssignment) {
                    ...
                    if (sectionAssignment.floor && parseInt(sectionAssignment.floor) > 0) {
                    ...
                    <div class="records-main">
                            ...
                            <div class="info-grid">
                                <div class="info-item">
                                    ...
                                    <div class="info-content">
                                        ...
                                        <span class="info-value">${escapeHtml(buildingText)}</span>
                                    </div>
                                </div>
                                ...
                                <div class="info-item">
                                    ...
                                    <div class="info-content">
                                        ...
                                        <span class="info-value">${escapeHtml(floorText && roomText ? floorText + ' / ' + roomText : (floorText || roomText || 'N/A'))}</span>
                                    </div>
                                </div>
                                ...
                                <div class="info-item">
                                    ...
                                    <div class="info-content">
                                        ...
                                        <span class="info-value">${escapeHtml(yearText)}</span>
                                    </div>
                                </div>
                                ...
                                <div class="info-item">
                                    ...
                                    <div class="info-content">
                                        ...
                                        <span class="info-value">${escapeHtml(sectionText)}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="info-card financial-card">
                            ...
                            <div class="financial-grid">
                                <div class="financial-item ${paymentStatus === 'owing' ? 'status-warning' : 'status-success'}">
                                    <div class="financial-icon">
                                        <i class="bi ${paymentStatus === 'owing' ? 'bi-exclamation-triangle' : 'bi-check-circle'}"></i>
                                    </div>
                                    <div class="financial-content">
                                        <span class="financial-label">Outstanding Balance</span>
                                        <span class="financial-value">
                                            ${paymentStatus === 'owing' && owingAmount ? '₱' + parseFloat(owingAmount).toFixed(2) : (paymentStatus === 'owing' ? 'Amount pending' : '₱0.00')}
                                        </span>
                                    </div>
                                </div>
                                ${sanctions || (sanctionDays !== null && sanctionDays > 0) || sanctionText.includes('days') || sanctionText === 'Yes' || sanctionText === 'Expired' ? `
                                <div class="financial-item status-warning">
                                    ...
                                    <div class="financial-content">
                                        ...
                                        <span class="financial-value">
                                            ${sanctionDays !== null && sanctionDays > 0 ? `<span class="days-remaining">${sanctionDays}</span> days remaining` : (sanctionText.includes('days') ? `<span class="days-remaining">${escapeHtml(sanctionText)}</span>` : (sanctionText === 'Expired' ? '<span class="text-muted">Sanction expired</span>' : (sanctionText === 'Yes' ? `<span class="text-warning">${escapeHtml(sanctionText)}</span>` : `<span class="days-remaining">${escapeHtml(sanctionText)}</span>`)))}
                                        </span>
                                        ${sanctionNote ? `<span class="sanction-note">Note: ${escapeHtml(sanctionNote)}</span>` : ''}
                                    </div>
                                </div>
                                ` : ''}
                            </div>
        ...
        contentArea.innerHTML = '<div class="alert alert-danger">Error loading records: ' + escapeHtml(error.message) + '</div>';
    }
}
...
async function loadAnnouncementsView() {
    ...
    try {
        ...
        // Setup filter form
        document.getElementById('announcementsFilterForm').addEventListener('submit', (e) => {
            e.preventDefault();
...
async function loadEvaluationView() {
    ...
    try {
        ...
        if (teacherId && teacherName) {
            ...
            await loadEvaluationForm(parseInt(teacherId), decodeURIComponent(teacherName), decodeURIComponent(subject || ''));
            return;
        }
            ...
            <div class="records-container">
                ...
                <div class="records-main">
                    <div class="info-card">
                        <div class="card-header-modern">
                            <i class="bi bi-people"></i>
                            <h3>Teachers in Section: ${escapeHtml(studentSection)}</h3>
                        </div>
        `;
                ...
                html += `
                    <div class="teacher-card-modern">
                        ...
                        <div class="teacher-subjects">
                            ...
                            <ul>
                                ${teacher.subjects.map(s => `<li>${escapeHtml(s)}</li>`).join('')}
                            </ul>
                        </div>
                        ${!teacher.evaluated ? `
                            <div class="teacher-card-actions">
                                <a href="?view=evaluation&teacher_id=${teacher.id}&teacher_name=${encodeURIComponent(teacher.name)}&subject=${encodeURIComponent(teacher.subjects.join(', '))}" class="btn btn-primary evaluation-btn">
        ...
        contentArea.innerHTML = '<div class="alert alert-danger">Error loading evaluation: ' + escapeHtml(error.message) + '</div>';
    }
}
...
async function loadEvaluationForm(teacherId, teacherName, subject) {
    ...
    try {
        ...
        let html = `
            <div class="records-container">
                <div class="records-header">
                    ...
                    <p class="records-subtitle">Evaluate ${escapeHtml(teacherName)}</p>
                </div>
                <div class="records-main">
                    <div class="info-card">
                        <div class="card-header-modern">
                    ...
                    <form id="evaluationForm">
                        ...
                        <input type="hidden" name="teacher_name" value="${escapeHtml(teacherName)}">
                        <input type="hidden" name="subject" value="${escapeHtml(subject)}">
                        <input type="hidden" name="student_section" value="${escapeHtml(studentSection)}">
        ...
        contentArea.innerHTML = '<div class="alert alert-danger">Error loading evaluation form: ' + escapeHtml(error.message) + '</div>';
    }
}
...
async function loadGradesView() {
    ...
    try {
                        ...
                        html += `
                            <div class="grades-semester-section">
                                <div class="semester-header">
                                    ...
                                    <span class="semester-badge">${gradesList.length} ${gradesList.length === 1 ? 'subject' : 'subjects'}</span>
                                </div>
                                <div class="grades-grid">
                        `;
                        
                        gradesList.forEach(grade => {
                            const grades = [grade.prelim_grade, grade.midterm_grade, grade.finals_grade].filter(g => g != null && g !== '');
                            const average = grades.length > 0 ? (grades.reduce((a, b) => parseFloat(a) + parseFloat(b), 0) / grades.length).toFixed(2) : null;
                            
                            html += `
                                <div class="grade-card-modern">
                                    <div class="grade-card-header-modern">
                                        <div class="grade-subject-info">
                                            <h5 class="grade-subject-name">${escapeHtml(grade.subject || 'Unknown')}</h5>
                                            ${grade.instructor ? `
                                            <p class="grade-instructor">
                                                <i class="bi bi-person-badge"></i>
                                                <span>${escapeHtml(grade.instructor)}</span>
                                            </p>
                                            ` : ''}
                                        </div>
                                        ${average !== null ? `
                                    ...
                                    <div class="grade-details-modern">
                                        <div class="grade-detail-item">
                                            ...
                                            <span class="grade-number">${grade.prelim_grade != null ? parseFloat(grade.prelim_grade).toFixed(2) : '—'}</span>
                                        </div>
                                        <div class="grade-detail-item">
                                            <span class="grade-period">
                                                <i class="bi bi-circle-fill"></i>
                                            ...
                                            <span class="grade-number">${grade.midterm_grade != null ? parseFloat(grade.midterm_grade).toFixed(2) : '—'}</span>
                                        </div>
                                        <div class="grade-detail-item">
                                            <span class="grade-period">
                                                <i class="bi bi-circle-fill"></i>
                                            ...
                                            <span class="grade-number">${grade.finals_grade != null ? parseFloat(grade.finals_grade).toFixed(2) : '—'}</span>
                                        </div>
                                    </div>
                                </div>
                            `;
        ...
        const studyLoadArray = Array.isArray(studyLoad) ? studyLoad : (studyLoad ? [studyLoad] : []);
        const assignment = await StudentAPI.getAssignment().catch(() => null);
        
        if (studyLoadArray.length > 0) {
            const yearLabel = assignment?.year ? (isNaN(assignment.year) ? assignment.year : formatOrdinal(parseInt(assignment.year)) + ' Year') : 'Not assigned';
            ...
            const semesters = [...new Set(studyLoadArray.map(s => s.semester).filter(s => s && s.trim() !== ''))];
            const formattedSemesters = semesters.map(formatSemester);
            const semesterLabel = formattedSemesters.length > 0 ? formattedSemesters.join(', ') : 'Not set';
                            ...
                            <tbody>
                                    ...
                                    <td colspan="4">
                                        <div class="row g-2">
                                            <div class="col-md-6">
                                                ...
                                                <span>${escapeHtml(currentUser?.school_id || 'Not set')}</span>
                                            </div>
                                            <div class="col-md-6">
                                                <strong>Full Name:</strong>
                                                <span>${escapeHtml(currentUser?.full_name || currentUser?.username || 'Not set')}</span>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
                                <tr>
                                    <td colspan="4">
                                        <div class="row g-0">
                                            <div class="col-md-3">
                                                <strong>Course:</strong> <span>${escapeHtml(courseLabel)}</span>
                                            </div>
                                            <div class="col-md-3">
                                                <strong>Major:</strong> <span>${escapeHtml(majorLabel)}</span>
                                            </div>
                                            <div class="col-md-3">
                                                <strong>Year Level:</strong> <span>${escapeHtml(yearLabel)}</span>
                                            </div>
                                            <div class="col-md-3">
                                                <strong>Section:</strong> <span>${escapeHtml(sectionLabel)}</span> <strong class="ms-1">Semester:</strong> <span>${escapeHtml(semesterLabel)}</span>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
            ...
            studyLoadArray.forEach(subject => {
                ...
                // Debug teacher field
                console.log('Subject:', subject.subject_code, 'Teacher field:', subject.teacher, 'Type:', typeof subject.teacher);
                
                const teacherDisplay = subject.teacher && subject.teacher.trim() !== '' 
        ...
        contentArea.innerHTML = '<div class="alert alert-danger">Error loading grades: ' + escapeHtml(error.message) + '</div>';
    }
}
...
function normalizeSemester(value) {
    ...
    const lower = raw.toLowerCase();
    const firstAliases = ['1', '1st', 'first', 'first semester', 'sem 1', 'semester 1', '1st semester'];
    for (const alias of firstAliases) {
        if (lower === alias) {
            return 'First Semester';
        }
    }
    if (lower.includes('first') || lower.includes('1st') || lower.includes('sem 1')) {
        return 'First Semester';
    }
    const secondAliases = ['2', '2nd', 'second', 'second semester', 'sem 2', 'semester 2', '2nd semester'];
    for (const alias of secondAliases) {
        if (lower === alias) {
            return 'Second Semester';
        }
    }
    if (lower.includes('second') || lower.includes('2nd') || lower.includes('sem 2')) {
        return 'Second Semester';
    }
    return raw.charAt(0).toUpperCase() + raw.slice(1);
}
...
async function loadTransparencyView() {
    ...
    try {
                    ...
                    <div class="project-card-modern">
                        <div class="project-card-header">
                            <div class="project-title-section">
                                <h4 class="project-title">${escapeHtml(proj.name || 'Unnamed Project')}</h4>
        ...
        contentArea.innerHTML = '<div class="alert alert-danger">Error loading projects: ' + escapeHtml(error.message) + '</div>';
    }
}
...
async function loadSettingsView() {
    ...
    try {
        ...
        if (success) {
            ...
            html += `
                <div class="alert alert-danger alert-dismissible fade show" role="alert">
                    <i class="bi bi-exclamation-triangle me-2"></i>An error occurred: ${escapeHtml(error)}
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                </div>
            `;
        }
                        ...
                        <div class="settings-profile-section">
                            <div class="profile-image-container">
                                <div class="profile-image-wrapper">
                                    <img id="preview" src="${escapeHtml((() => {
                                        let imgPath = currentUser?.image_path || 'images/sample.jpg';
                                        if (imgPath && imgPath.startsWith('/TCC/public/')) {
                                            imgPath = imgPath.replace('/TCC/public/', '');
                                        } else if (imgPath && imgPath.startsWith('/TCC/database/pictures/')) {
                                            imgPath = imgPath.replace('/TCC/database/pictures/', 'database/pictures/');
                                        }
                                        return imgPath;
                                    })())}" class="profile-preview-large" alt="Profile" />
                            ...
                            <div class="settings-form-fields">
                                <div class="settings-field">
                                    ...
                                    <input id="username" name="username" class="settings-input" value="${escapeHtml(currentUser?.username || '')}" required />
                                ...
                                <div class="settings-field">
                                    ...
                                    <input id="full_name" name="full_name" class="settings-input" value="${escapeHtml(currentUser?.full_name || '')}" required />
                                ...
                                <div class="settings-field">
                                    ...
                                    <input id="confirm_password" name="confirm_password" type="password" class="settings-input" placeholder="Confirm new password" />
                                </div>
                                ...
                                <div class="settings-actions">
                                    ...
                                    <a href="#" data-view="records" class="btn btn-outline-secondary">
        ...
        document.getElementById('settingsForm').addEventListener('submit', async (e) => {
            ...
            formData.delete('confirm_password'); // Remove confirm_password as backend doesn't need it
            ...
            try {
                ...
                if (result.success) {
                    // Always refresh user data from server to get latest image_path
                    try {
                        const checkResult = await AuthAPI.checkSession();
                        if (checkResult.authenticated && checkResult.user) {
                            console.log('Refreshing user data, new image_path:', checkResult.user.image_path);
                            currentUser = checkResult.user;
                            updateSidebar();
                        } else if (result.user) {
                            // Fallback to result.user if checkSession fails
                            console.log('Using result.user, image_path:', result.user.image_path);
                            currentUser = { ...currentUser, ...result.user };
                            updateSidebar();
                        }
                    } catch (refreshError) {
                        console.error('Error refreshing user data:', refreshError);
                        // Fallback to result.user if available
                        if (result.user) {
                            currentUser = { ...currentUser, ...result.user };
                            updateSidebar();
                        }
                    ...
                    // Show success message and reload after a short delay to ensure image is visible
                    alert('Profile updated successfully!');
                    // Small delay to let image load, then redirect
                    setTimeout(() => {
                        window.location.href = 'home.html?view=settings&success=1';
                    }, 500);
                } else {
        ...
        // Setup username/fullname availability checking
        ...
        if (usernameInput) {
            usernameInput.addEventListener('input', debounce(async function() {
                ...
                try {
                    const result = await AuthAPI.checkAvailability('username', val);
        ...
        if (fullInput) {
            fullInput.addEventListener('input', debounce(async function() {
                const val = this.value.trim();
                if (!val || val === currentUser?.full_name) {
                    document.getElementById('fullnameFeedback').style.display = 'none';
                    return;
                }
                try {
                    const result = await AuthAPI.checkAvailability('full_name', val);
                    const fb = document.getElementById('fullnameFeedback');
                    if (!result.available) {
                        fb.style.display = 'block';
                        fb.textContent = 'Full name already used';
                    } else {
                        fb.style.display = 'none';
                    }
                } catch (error) {
        ...
        contentArea.innerHTML = '<div class="alert alert-danger">Error loading settings: ' + escapeHtml(error.message) + '</div>';
    }
}
