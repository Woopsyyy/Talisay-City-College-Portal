...
export default function LevelSectionFilter() {
  ...
  useEffect(() => {
    const fetchFilters = async () => {
      try {
        ...
        setAllSectionsData(Array.isArray(data) ? data : []);
        ...
        const formattedYears = uniqueYears.map(year => ({
          id: year.toString(),
          label: isNaN(year) ? year : `Year ${year}`
        }));
        setYearLevels(formattedYears);

        
        const uniqueSections = [...new Set(data.map(item => item.name))].sort();
  ...
  useEffect(() => {
    ...
    if (selectedLevel !== 'all') {
      relevantSections = allSectionsData.filter(item => item.year.toString() === selectedLevel);
    }

    const uniqueSectionNames = [...new Set(relevantSections.map(item => item.name))].sort();
