...
const PaymentRecordsView = () => {
  ...
  const loadRecords = async (activeFilters = filters) => {
    try {
      ...
      showToast(error.message || "Failed to load payment records.", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const openStudentRecord = async (row) => {
    ...
    try {
      ...
      const details = await AdminAPI.getPaymentRecordForStudent(row.user_id, {
        school_year: filters.school_year || row.school_year || null,
      });
      setSelectedRecord(details || row);
    } catch (error) {
      showToast(error.message || "Failed to open payment record.", "error");
    } finally {
    ...
    <Container>
      <Header>
        <div>
          ...
          <p>Track student receipts and payment history by scope.</p>
        </div>
      </Header>
            ...
            {detailsLoading ? (
              ...
              <ModalBody>
                <SummaryGrid>
                  <SummaryCard>
                    <label>Status</label>
                    <strong>{String(selectedRecord?.payment_status || "paid")}</strong>
                  </SummaryCard>
                  <SummaryCard>
                    <label>Amount Lacking</label>
                    <strong>PHP {formatMoney(selectedRecord?.amount_lacking || 0)}</strong>
                  </SummaryCard>
                  <SummaryCard>
                    <label>Receipts Logged</label>
                    <strong>{selectedRecord?.receipt_count || 0}</strong>
                  </SummaryCard>
                </SummaryGrid>

                <Timeline>
                  {(selectedRecord?.receipts || []).length === 0 ? (
                    <TimelineItem>No payment events logged for this scope.</TimelineItem>
                  ) : (
                    selectedRecord.receipts.map((event) => (
